...
eval4cls2 <- function(model, dataset, yname, modelname, datasetname,
  ...
  rocresult <- predresult %>%
    ...
    mutate(
      ...
      curvelab = paste0("ROCAUC=", 
                      round(rocauc, 4),
                      "(", round(as.numeric(pROC::ci.auc(proc))[1], 4), 
                      "-", round(as.numeric(pROC::ci.auc(proc))[3], 4),
                      ")")
    )
  ...
  prresult <- predresult %>%
    ...
    mutate(
      ...
      curvelab = paste0("PRAUC=", 
                        round(PRAUC, 4),
                        "(", round(quantile(pr_boot2, 0.025), 4), 
                        "-", round(quantile(pr_boot2, 0.975), 4),
                        ")")
    )
  ...
  cmplot <- cmresult$table %>%
    ...
    geom_text(aes(label = Freq), family = "serif") +
...
bestcv4cls2 <- function(wkflow, tuneresult, hpbest, yname, 
  ...
  cvroc <- predcv %>%
    ...
    left_join(evalcv[, c(1,2,4)] %>% filter(.metric == "roc_auc"), 
              by = "id") %>%
    mutate(idAUC = paste(id, " ROC_AUC:", round(.estimate, 4)),
  ...
  cvpr <- predcv %>%
    ...
    left_join(evalcv[, c(1,2,4)] %>% filter(.metric == "pr_auc"), 
              by = "id") %>%
    mutate(idAUC = paste(id, " PR_AUC:", round(.estimate, 4)),
           idAUC = forcats::as_factor(idAUC)) %>%
...
# dalex变量重要性
viplot <- function(explainer){
  set.seed(1220)
  vip <- DALEX::model_parts(
    explainer,
    type = "ratio"
  )
  pi <- plot(vip, show_boxplots = FALSE) +
    labs(subtitle = NULL) +
...
shap4cls2 <- function(finalmodel, predfunc, datax, datay,
  ...
  shapvip <- shap$shapley_values %>%
    as.data.frame() %>%
    dplyr::mutate(id = 1:n()) %>%
    pivot_longer(cols = -(ncol(datax)+1), 
                 names_to = "feature",
                 values_to = "shap") %>%
    dplyr::group_by(feature) %>%
    dplyr::summarise(shap.abs.mean = mean(abs(shap), na.rm = T)) %>%
  ...
  shapvipplot <- shapvip %>%
    ...
    scale_x_continuous(expand = c(0, 0),
                       limits = c(0, max(shapvip$shap.abs.mean)*1.05)) +
  ...
  shapvipplot_unity <- shapvip_unity %>%
    ...
    scale_fill_distiller(palette = "YlOrRd", direction = 1) +
    scale_x_continuous(expand = c(0, 0),
                       limits = c(0, max(shapvip_unity$importance)*1.05)) +
  ...
  shapplot_unity <- shapdata_unity %>%
    ...
    scale_color_gradient2(
      ...
      guide = guide_colorbar(
        barwidth = 0.5,
        barheight = min(length(unique(shapvip_unity$feature)),
                        15), 
        ticks = F,
        title.position = "right",
        title.hjust = 0.5
