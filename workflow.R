...
# 读取只有一列且有表头的文件
DEG_data <- read.table("intersectGenes_edgeR.txt", header=TRUE, sep="\t", check.names=FALSE, stringsAsFactors=FALSE)
#转化为matrix
dimnames=list(rownames(data), colnames(data))
data=matrix(as.numeric(as.matrix(data)), nrow=nrow(data), dimnames=dimnames)
#正常和肿瘤数目,第14,15字符,01-09是癌症，10-19是正常，20-29是癌旁
...
# 假设从训练集的评估中提取的 cutoff
train_cutoff <- evaltrain_lasso$cutoff

# 在测试集的评估中应用训练集的 cutoff
