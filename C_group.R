...
for (i in group) {
  #找样本间差异基因C15L
  pbmc.C15L <- FindMarkers(pbmc, ident.1 = i, ident.2 = "Ad1R", group.by = 'group',
  ...
  markGenes = unique(Top.markers$gene)[sample(1:length(unique(Top.markers$gene)),40,
                                              replace = F)]
  #获取差异基因比较样本的数量
  ...
  png(paste0(plotpath,"diff_gene_GO_",i,".png",sep = ""),width = 18,height = 6, units = "in", res = 800)
