...
for (i in group) {
  #在E:/C组数据/单细胞结果/亚群拟时序分析/这个文件夹中创建一个叫k的文件夹
  path <- paste0("E:/C组数据/cellphoneDB准备文件备份/", i)
  dir.create(path)
  #提取每个样本
  cluster <- i
  exp = pbmc[, pbmc@meta.data$group %in% cluster]#抽组
  exp@meta.data$group <- droplevels(exp@meta.data$group)
