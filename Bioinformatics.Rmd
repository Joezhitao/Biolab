...
# 提取TBC1D16基因的表达值
tbc1d16_expression = data["TBC1D16", ]

# 确定高低表达的阈值，可以使用中位数或其他统计量
threshold = median(tbc1d16_expression, na.rm = TRUE)

# 根据阈值将样本分为高表达和低表达
group = ifelse(tbc1d16_expression > threshold, "High", "Low")
...
write.table(data.frame(ID = rownames(outdiff), outdiff),
            file = "E:/CRC/TBC1D16_High_Low.diff.DESeq2.txt",
...
for (i in input_gene) {
  ...
  write.table(GO, file=paste0(base_dir,"/","GO_base_on_unicox.txt"), sep="\t", quote=F, row.names = F)
  eGo <- GO
...
#读入指定基因
gene_ppi = c('CARD14', 'CDC16', 'PPM1L', 'RAB4A', 'TBC1D1', 'TBC1D16', 'TBC1D19', 'TBC1D20')
for (i in gene_ppi) {
  # 将gene symbol转换为基因id
  entrezIDs = mget(i, org.Hs.egSYMBOL2EG, ifnotfound = NA)
  entrezIDs = as.character(entrezIDs)
  ...
  # 在绘图之前检查 KEGG 数据框
  if (nrow(KEGG) > 0) {
    # 确保 pvalue 和 qvalue 没有 NA
    KEGG = KEGG[!is.na(KEGG$pvalue) & !is.na(KEGG$qvalue), ]
    
    # 确保 KEGG 数据框中有数据
    if (nrow(KEGG) > 0) {
      # 柱状图
      bar <- barplot(kk, drop = TRUE, showCategory = showNum, label_format = 130, color = colorSel)
      pdf(file = paste0(save_dir, "/", i, "_KEGGbarplot.pdf"), width = 7, height = 7)
      print(bar)
      dev.off()
      
      # 气泡图
      dot <- dotplot(kk, showCategory = showNum, orderBy = "GeneRatio", label_format = 130, color = colorSel)
      pdf(file = paste0(save_dir, "/", i, "_KEGGbubble.pdf"), width = 7, height = 7)
      print(dot)
      dev.off()
      } else {
        message("No significant pathways found for gene ", i)
      }
    } else {
      message("No KEGG enrichment results for gene ", i)
    }
...
```{r single_gene_pairwise, message=TRUE, warning=TRUE, include=FALSE}
#加载
library("limma")
library("ggpubr")
#设置工作目录
...
for (i in gene_ppi) {
  ...
  # 检查基因是否在正常和肿瘤数据中
  if (!(gene %in% rownames(normal))) {
    warning(paste("Gene", gene, "not found in normal data"))
    next
  }
  if (!(gene %in% rownames(tumor))) {
    warning(paste("Gene", gene, "not found in tumor data"))
    next
  }
  
  # 对正常数据进行整理
  normal_gene = normal[gene, , drop = FALSE]
  normal_gene = as.matrix(t(normal_gene))
  rownames(normal_gene) = gsub("(.*?)\\-(.*?)\\-(.*?)\\-(.*?)\\-.*", "\\1\\-\\2\\-\\3", rownames(normal_gene))
  
  # 对肿瘤数据进行整理
  tumor_gene = tumor[gene, , drop = FALSE]
  tumor_gene = as.matrix(t(tumor_gene))
  rownames(tumor_gene) = gsub("(.*?)\\-(.*?)\\-(.*?)\\-(.*?)\\-.*", "\\1\\-\\2\\-\\3", rownames(tumor_gene))
  
  # 合并数据
  samSample = intersect(rownames(normal_gene), rownames(tumor_gene))
  
  # 检查是否有交集样本
  if (length(samSample) == 0) {
    warning(paste("No common samples for gene", gene))
    next
  }
  
  normal_gene = normal_gene[samSample, , drop = FALSE]
  tumor_gene = tumor_gene[samSample, , drop = FALSE]
  data_combined = cbind(normal_gene, tumor_gene)
  
  # 确保数据框的列数足够
  if (ncol(data_combined) < 2) {
    warning(paste("Insufficient data for gene", gene))
    next
  }
  
  data_combined = as.data.frame(data_combined[, c(1, 2)])
  colnames(data_combined) = c("Normal", "Tumor")
  
  # 绘制图形
  ppiplot <- ggpaired(data_combined, cond1 = "Normal", cond2 = "Tumor", fill = c("red", "blue"), palette = "jco",
           xlab = "", ylab = paste0(gene, " expression")) +
    stat_compare_means(paired = TRUE, label = "p.format", label.x = 1.35)
  pdf(file = paste0("E:/CRC/TBC1D16/ppidiff/pairDiff_", i, ".pdf"), width = 5.5, height = 5)
  print(ppiplot)
  dev.off()
}
...
```{r gene_clinical_heatmap, message=TRUE, warning=TRUE, include=FALSE}
#加载R包
library(limma)
...
#cli$ID <- rownames(cli)
#ID列提取到第一列
#cli=cli[,c(ncol(cli),1:(ncol(cli)-1))]
#cli[,"age"]=ifelse(cli[,"age"]=="unknow", "unknow", ifelse(cli[,"age"]>65,">65","<=65"))
#年龄按结直肠癌筛查与早诊早治方案（2024年版）中的年龄分组标准进行分组
cli[,"age"] <- ifelse(cli[,"age"] == "unknow", 
                            "unknow", 
                            ifelse(cli[,"age"] < 50, 
                                   "≤49岁", 
                                   ifelse(cli[,"age"] <= 59, 
                                          "50-59岁", 
                                          "≥60岁")))
#BMI按结直肠癌筛查与早诊早治方案（2024年版）中的BMI分组标准进行分组
cli[,"BMI"] <- ifelse(cli[,"BMI"] == "unknow", 
                            "unknow", 
                            ifelse(cli[,"BMI"] < 23, 
                                   "<23kg/m²", 
                                   "≥23kg/m²"))
...
#输出热图,*号说明相关性有统计学意义
pdf(file="TBC1D16_heatmap.pdf", width=25, height=25)
#png(file="heatmap.png", width=25, height=25, units="in", res=800)
draw(Hm, merge_legend = TRUE, heatmap_legend_side = "bottom", annotation_legend_side = "bottom")
dev.off()
```

## 突变数据分析
```{r mutation, message=TRUE, warning=TRUE, include=FALSE}
