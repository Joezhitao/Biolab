...
dir.create(file.path(base_dir, "uniCox", "limma"), showWarnings = FALSE)  # 创建 "E:/CRC/uniCox/limma" 文件夹
save_dir <- file.path(base_dir, "uniCox", "limma")
# 设置工作目录
setwd(base_dir)
#读取表达文件
data=read.table("TCGA_CRC_count.txt", header=T, sep="\t", check.names=F,row.names = 1)
...
write.table(outTab, file="uniCox.xlsx", sep="\t", row.names=F, quote=F)
...
save_dir <- file.path(base_dir, "GO", "limma")
#读入指定基因
input_gene=c("ABCD3","ACOX1","CD1B","CDK5R2","CPT2","GLP2R","HPCAL4","HPSE2","KCNQ4","MAFA","OTOP3",
             "TPM1","VGF")
for (i in input_gene) {
  #将gene symbol转换为基因id
  #org.Hs.eg为人的物种
...
## 生存分析_for循环跑所有uniCox有意义基因并且只挑出生存分析有意义
...
for (i in gene) {
  #设置保存路径
  filepath <- paste0("E:/CRC/SURV/genesurv_pvalue/")
  #读入生存数据
  cli=read.table("time_CRC.txt", header=T, sep="\t", check.names=F, row.names=1)
  #生存时间以年为单位
  cli$time=cli$time/365
...
pVal <- ifelse(rt$pvalue<0.001, "<0.001", sprintf("%.3f", rt$pvalue))
#绘制森林图
pdf(file="multiCoxforest.pdf", width = 12,height = nrow(rt)/13+5)
n <- nrow(rt)
nRow <- n+1
ylim <- c(1,nRow)
layout(matrix(c(1,2),nc=2),width=c(3,2.5))
