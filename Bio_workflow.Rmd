...
geneNum=50
outdiff$gene <- rownames(outdiff)
outdiff=outdiff[order(as.numeric(as.vector(outdiff$log2FoldChange))),]
diffGeneName=as.vector(outdiff[,7])
diffLength=length(diffGeneName)
hmGene=c()
if(diffLength>(2*geneNum)){
...
```{r Boruta, message=TRUE, warning=TRUE}
# Joe_2024_10_12
rm(list=ls())
...
##模型1 coxph
```{r coxph,, message=TRUE, warning=TRUE}
library(tidyverse)
library(survival)
library(mlr3verse)
library(mlr3proba)
...
cindex_bootci(learner_coxph, traindata3)
...
cindex_bootci(learner_coxph, testdata3)
